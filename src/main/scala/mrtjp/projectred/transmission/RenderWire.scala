...
object RenderWire {
  ...
  def getOrGenerateModel(key: Int) = {
    ...
    if (m == null) wireModels(key) = {
      m = WireModelGen.getThreadGenerator().generateModel(key, false); m
    }
    m
  }
  def getOrGenerateInvModel(thickness: Int) = {
    ...
    if (m == null) invModels(thickness) = {
      m = WireModelGen.getThreadGenerator().generateInvModel(thickness); m
    }
    m
  }
...
object WireModelGen {
  ...
  private val threadGenerator = ThreadLocal.withInitial(new Supplier[WireModelGen]() {
    override def get(): WireModelGen = new WireModelGen()
  })

  def getThreadGenerator() = threadGenerator.get()
}
